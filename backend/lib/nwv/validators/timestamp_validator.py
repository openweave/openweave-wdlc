...
#      validates and enforces timestamp constraints.
#

"""Validator for timestamp constraints."""
...
class TimestampValidator(validator.VisitorValidator):
  """Validator for timestamp constraints."""

  def visit_Field(self, field):
    if not (field.struct_type and
            field.struct_type.full_name == "google.protobuf.Timestamp"):
      return
    ...
    if signed is None or precision is None or width is None:
      self.add_failure("Timestamp constraints, if specified, "
    ...
    if not ((signed and precision == 0.001 and width == 64) or
      ...
      self.add_failure("Invalid timestamp constraints, (must be int64 "
                       "milliseconds or uint32 seconds) \n"
                       "go/phoenix-idl-standard-types has more information")


process = TimestampValidator.process
